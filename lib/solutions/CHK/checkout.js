...
const products = {
    ...
    S: {
        price: 20,
        offers: [
            { type: 'group', group: ['S', 'T', 'X', 'Y', 'Z'], required: 3, groupPrice: 45 }
        ]
    },
    T: {
        price: 20,
...
const applyGroupedDiscounts = (quantities) => {
    ...
    for (const sku of Object.keys(groupedQuantities)) {
        ...
        for (const offer of offers) {
            if (offer.type === 'group') {
                let groupedProducts = []
                const quantitiesTmp = { ...groupedQuantities }

                // Get all eligible group skus
                for (const groupSku of offer.group) {
                    if (groupedQuantities[groupSku] > 0) {
                        while (quantitiesTmp[groupSku] > 0) {
                            groupedProducts.push({ sku: groupSku, price: products[groupSku].price })
                            quantitiesTmp[groupSku] -= 1
                        }
                    }
                }

                // Order by highest prices and only take the required number of skus
                groupedProducts = groupedProducts.sort((a, b) => b.price - a.price).splice(0, offer.required)

                // Add prices to total
                if (groupedProducts.length >= offer.required) {
                    for (const product of groupedProducts) {
                        groupedQuantities[product.sku] -= 1
                    }
