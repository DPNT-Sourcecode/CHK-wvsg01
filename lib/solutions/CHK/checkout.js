...
module.exports = function (skus) {
    ...
    for (const sku of skus.split('')) {
        ...
        console.log(sku, productQuantities[sku] >= product.promo.quantity)

        // If the product has a promo, handle it
        if (product.promo && productQuantities[sku] >= product.promo.quantity) {
            const amountDifference = (product.price * product.promo.quantity) - product.promo.price
