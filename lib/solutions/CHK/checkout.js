...
const products = {
    ...
    S: {
        price: 30,
        offers: [
            { quantity: 1, type: 'group', group: ['T', 'X', 'Y', 'Z'], required: 2, price: 45 }
        ]
    },
    T: {
        price: 20,
        offers: [
            { quantity: 1, type: 'group', group: ['S', 'X', 'Y', 'Z'], required: 2, price: 45 }
        ]
    },
    U: {
        price: 40,
    ...
    X: {
        price: 90,
        offers: [
            { quantity: 1, type: 'group', group: ['S', 'T', 'Y', 'Z'], required: 2, price: 45 }
        ]
    },
    Y: {
        price: 10,
        offers: [
            { quantity: 1, type: 'group', group: ['S', 'T', 'X', 'Z'], required: 2, price: 45 }
        ]
    },
    Z: {
        price: 50,
        offers: [
            { quantity: 1, type: 'group', group: ['S', 'T', 'X', 'Y'], required: 2, price: 45 }
        ]
    },
}
    ...
    for (const sku of Object.keys(quantities)) {
        ...
        while (quantities[sku] !== 0) {
            ...
            for (const offer of offers) {
                ...
                if (offer.type === 'group') {
                    let requiredProducts = offer.required

                    for (const groupProduct of offer.group) {
                        if (quantities[groupProduct] > 0) {
